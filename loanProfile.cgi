...
#def addPopup(docname,available,verified,timestamp,uploader,auditor,provider):
def addPopup(doc):
	displayDoc="-"
	docname=""
	available="-"
	...
	print "							<thead class='defaultHeadingPlain'>"
	print "							<tr>"
	...
	if (complianceData):
		...
		print "					<object style='align:center;'><embed width='700px' height='auto' src='samplepolicy.pdf' type='application/pdf'></object>"
		print "				</div>"
	#=========== if complianceData
	print "					</div>"
	print "				</div>"
...
def displayLoanData():
	...
	profile=1
	if (profile==0):
	        print "<h2>mChain Information</h2>"
	...
	if (profile==0):
	        ...
	        print "        <td class='undefined'><a href='/mchain/mortgageCustomerTransactions.cgi?address=%s'>%s</a></td>" %(customer['address'],customer['address'])
	        print "</tr>"
	        print "</tbody>"
	        print "</table>"
...
if not form:
    ...
    if (password==passkey):
      ...
      if (customerAddress == "False"):
        ...
        txid=getProfileTxid(loanNumber,phase)
