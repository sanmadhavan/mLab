			...
			for fdata in child:
				...
				if (str(fdata.tag) == "insurance"):
					insuranceStatus=str(fdata.text)
                        if any (re.findall (r'Servicing|Transfer',loanStatus,re.IGNORECASE)):
				return 2
			elif (insuranceStatus == "Pending"):
				return 1
...
def displayLoanData():
	...
	if (iStatus==1):
        	print "<h2>Portfolio&nbsp&nbsp<span style='font-size:15px;'>To share this file, <a href='/mortgage/viewPortfolio.cgi?loannumber=%s&phase=Insurance' target='popup' onclick=\"window.open('/mortgage/viewPortfolio.cgi?loannumber=%s&phase=Insurance','Portfolio','width=1400,height=800')\" >cick here</a></span></h2>" % (loanNum,loanNum)
	elif (iStatus==2):
        	print "<h2>Portfolio&nbsp&nbsp<span style='font-size:15px;'>To share this file, <a href='/mortgage/viewPortfolio.cgi?loannumber=%s&phase=Servicing' target='popup' onclick=\"window.open('/mortgage/viewPortfolio.cgi?loannumber=%s&phase=Servicing','Portfolio','width=1400,height=800')\" >cick here</a></span></h2>" % (loanNum,loanNum)
	else:
	...
	for child in root:
        ...
        for child in root:
           if (child.tag == "1Payments"):

              #print "<table class='table table-bordered'>"
              print "<div class='navHeading'>"
              #print "<span class='alignLeft sideHeading' data-toggle='collapse' href='#payments1'>Payments</span>"
              print "</div>"
              print "<div id='payments1' class='panel-collapse collapse'>"
              for fdata in child:
                 print "<table class='table table-bordered'>"
                 print "<thead class='defaultHeading'>"
                 print "<tr>"
                 print "<th> Sl No.</th> " 
                 print "<th> %s </th> " %(str (fdata.attrib.get('display')))
                 print "</tr>"
                 print "</thead>"
                 print"<tbody>"
                 iterator=0
                 for docs in fdata:
                    iterator=iterator+1
                    print "<tr>"
                    print "<td>%s</td>" % (str(iterator))
                    print "<td>%s</td>" % (str(docs.text))
                    print "</tr>"
                 print "</tbody>"
                 print "</table>"
                 print "</div>"
# ---------------------------------------------------------------------------------------
        for child in root:
		 ...
		 ##------
		     ...
		     # txndataloop ends here
                 #emiMonth loop ends here
                 print "                   </tbody>"
