...
# -----------------------------     mchainLib.py   ----------------------------
# customised the OP_RETURN library
# ----------------------------------------------------------------------------- 


import sys,subprocess, json, time, random, os.path, binascii, struct, string, re, hashlib, urllib2
...
OP_RETURN_MAX_BYTES=8388608 # maximum bytes in an OP_RETURN (40 as of Bitcoin 0.10)
#OP_RETURN_MAX_BLOCKS=10 # maximum number of blocks to try when retrieving data
OP_RETURN_MAX_BLOCKS=1000000000 # maximum number of blocks to try when retrieving data

OP_RETURN_NET_TIMEOUT=10 # how long to time out (in seconds) when communicating with bitcoin node


# User-facing functions
def OP_RETURN_sendwithmetadatafrom(bankaddress,customeraddress, send_amount, metadata, testnet=False):
	...
	#result=OP_RETURN_bitcoin_cmd('validateaddress', testnet, customeraddress)
	...
	#if metadata_len>8388608:
	#	return {'error': 'This library only supports metadata up to 8388608 bytes in size'}
	...
	return {'txid':"11111"}
        send_txid=OP_RETURN_bitcoin_cmd('sendwithmetadatafrom', testnet, bankaddress,customeraddress,send_amount,metadata)
...
def OP_RETURN_upload(send_address, send_amount, metadata, testnet=False):
	# Validate some parameters
	...
	print "Sanjeevs:Connected "

	result=OP_RETURN_bitcoin_cmd('validateaddress', testnet, send_address)
	if not ('isvalid' in result and result['isvalid']):
		return {'error': 'Send address could not be validated: '+send_address}
	print "Sanjeevs:Address validated "
	
	if isinstance(metadata, basestring):
		metadata=metadata.encode('utf-8') # convert to binary string

	metadata_len=len(metadata)
	print "Metadata length = "+str(metadata_len )

	print OP_RETURN_MAX_BYTES
	...
	print "Sanjeevs:Metadata check complete "

        send_txid=OP_RETURN_bitcoin_cmd('sendwithmetadata', testnet, send_address,send_amount,metadata)
        if not (isinstance(send_txid, basestring) and len(send_txid)==64):
                return {'error': 'Could not send the transaction'}

        print "Sendwithmetadata "
        print send_txid
        return {'txid': str(send_txid)}

def OP_RETURN_getLatestTxid(address, testnet=False):
	if not OP_RETURN_bitcoin_check(testnet):
		return {'error': 'Please check Bitcoin Core is running and OP_RETURN_BITCOIN_* constants are set correctly'	}

        
	latestTX=""
	blockheight= OP_RETURN_bitcoin_cmd('getblockcount',testnet=False)
        confimationCount=blockheight
	result=OP_RETURN_bitcoin_cmd('listaddresstransactions', testnet, address)
	for elt in result:
		if (elt['confirmations'] != blockheight):
  		  metadata1=elt['data']
		  if (metadata1):
                        if (int (elt['confirmations']) < confirmationCount):
                           confirmationCount=int (elt['confirmations'])
                           latestTX=elt['txid']
        return latestTX 

def OP_RETURN_getChain_transactions(address, testnet=False):
	if not OP_RETURN_bitcoin_check(testnet):
		return {'error': 'Please check Bitcoin Core is running and OP_RETURN_BITCOIN_* constants are set correctly'	}
	...
	for elt in result:
		if (elt['confirmations'] != blockheight):
  		  metadata1=elt['data']
		  if (metadata1):
			#print "Block Index : "+str(elt['blockindex'])
			#print "Confirmations : "+str(elt['confirmations'])
			#print "TxID : "+ str( elt['txid'])
			#print "\tMetadata : "+str(metadata1)
			#print "ELT = "
			#print elt
			blockhash=str(elt['blockhash'])
			block=OP_RETURN_bitcoin_cmd('getblock', testnet,blockhash)
			blockNumber=block['height']
			dateTime=time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(elt['time']))
			ChainRow=elt['txid']+"|"+str(elt['confirmations'])+"|"+str(blockNumber)+"|"+dateTime
			#transactionsList.append(elt['txid'])
			transactionsList.append(ChainRow)
	return {'txns':transactionsList}

def OP_RETURN_list_address_transactions(address, testnet=False):
	if not OP_RETURN_bitcoin_check(testnet):
		return {'error': 'Please check Bitcoin Core is running and OP_RETURN_BITCOIN_* constants are set correctly'	}
...
def OP_RETURN_get_transaction_data(txid,testnet=False):
	if not OP_RETURN_bitcoin_check(testnet):
		return {'error': 'Please check Bitcoin Core is running and OP_RETURN_BITCOIN_* constants are set correctly'	}

	result=OP_RETURN_bitcoin_cmd('getwallettransaction', testnet, txid)
	return result
...
def OP_RETURN_bitcoin_cmd(command, testnet, *args): # more params are read from here
	if OP_RETURN_BITCOIN_USE_CMD:
		...
		if not (len(port) and len(user) and len(password)):
			#conf_lines=open(os.path.expanduser('~')+'/POC/data/mchain/multichain.conf').readlines()
			conf_lines=open('/home/sanjeev/POC/data/mchain/multichain.conf').readlines()
		...
		try:
			...
			contents = raw_result
    			...
    			contents = error.read()
			result_array=json.loads(contents.decode('utf-8'))
			return result_array
